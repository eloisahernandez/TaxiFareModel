...
import joblib
import pandas as pd
from TaxiFareModel import gcparams
...
def get_test_data(nrows, data="s3"):
    ...
    if data == "local":
        ...
        df = pd.read_csv(f"gs://{gcparams.BUCKET_NAME}/{gcparams.BUCKET_TEST_DATA_PATH}")
    else:
        df = pd.read_csv(AWS_BUCKET_TEST_PATH, nrows=nrows)
    return df


def download_model(bucket=gcparams.BUCKET_NAME,
                   rm=True):
    client = storage.Client().bucket(bucket)

    blob = client.blob(gcparams.STORAGE_LOCATION)
